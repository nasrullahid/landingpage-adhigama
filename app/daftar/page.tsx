...
export default function Daftar() {
  ...
  async function handleSubmit() {
    ...
    if (!response.ok) {
      ...
      // router.push('/page/payment/success')
      import("react-facebook-pixel")
        .then((x) => x.default)
        .then((ReactPixel) => {
          ReactPixel.init(pixelID);
        ...
        <div className="bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-200 sm:rounded-xl sm:shadow-xl">
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center p-4 border border-gray-200 rounded dark:border-gray-700 mb-2">
              <p className='font-semibold text-lg sm:font-medium'>Paket Platinum</p>
            ...
            <div className='flex items-center justify-start mb-5'>
              <label className='w-full max-w-32' htmlFor='nama'>Nama Lengkap</label>
              <input id="nama" onChange={(e) => setNama(e.target.value)} className='appearance-none bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-800 rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline' type="text" />
            </div>
            <div className='flex items-center justify-start mb-5'>
              <label className='w-full max-w-32' htmlFor='email'>Email</label>
              <input id="email" onChange={(e) => setEmail(e.target.value)} className='appearance-none bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-800 rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline' type="email" />
            </div>
            <div className='flex items-center justify-start mb-5'>
              <label className='w-full max-w-32' htmlFor='phone'>No. phone</label>
              <input id="phone" onChange={(e) => setPhone(e.target.value)} className='appearance-none bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-800 rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline' type="number" />
            </div>
            <hr className='my-5' />
            ...
            <ul className='mb-6'>
              {loading ?
                Array.from({ length: 3 }).map((_, index) => (
                  <li key={index} > <Skeleton classAtr="dark:border-gray-600 border w-full h-20 mb-2" /></li>
                )) :
                <li>
                  <div className="">
                    {channels && channels?.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {item.bankName}
                            <img src={`/bank/${item.logo}`} alt={item.bankName} className='h-7' />
                          </label>
                        </div>
                      )
                      // }
            ...
            <div className='flex flex-col sm:flex-row items-center'>
              <button onClick={handleSubmit} disabled={isSubmited} className="px-5 py-3 bg-blue-600 hover:bg-blue-700 font-bold text-white w-full sm:w-auto rounded-md mb-2 mr-0 sm:mb-0 sm:mr-2">
                {isSubmited && (<>Loading ...</>)}
            ...
            {errors && (
              <p className='my-6 p-4 rounded-md bg-red-400 text-white text-sm border border-red-700'>{errors}</p>
            )}
            <p className='mt-4'>Dengan checkout, Anda setuju dengan <a href='/kebijakan-layanan' target='_blank' className='text-blue-600 font-semibold hover:underline'>Ketentuan Penggunaan</a> kami dan mengonfirmasi bahwa Anda telah membaca <a href='/kebijakan-privasi' target='_blank' className='text-blue-600 font-semibold hover:underline'>Kebijakan Privasi</a> kami.</p>
          </div>
        </div>
        {/* </div> */}
      </div>
